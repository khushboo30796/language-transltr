...
if(isset($_POST['l1']))
	...
	while($arr=$r->fetch_assoc())
		{$flag=1;
		$sql='select no_of_translations from articles where Title= :title';
		
		echo "</td><td>".$arr['jname']."</td><td>".$arr['title']."</td><td>".$arr['author_name']."</td><td>".$arr['acad_year']."</td><td>".$arr['pub_year']."</td><td>".$arr['issue']."</td><td>".$arr['vol']."</td><td>".$arr['pg_start']."</td><td>".$arr['pg_end']."</td><td><a href='".$arr['url']."'>View in English</a></td></tr>";
		}
	if ($flag==0)
